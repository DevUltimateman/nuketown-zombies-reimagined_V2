...
CustomRoundNumber() //original code by ZECxR3ap3r, modified it to my liking
{
	if(!isdefined(self.hud))
	{
		...
		self.hud = create_simple_hud();
		self.hudtext = create_simple_hud();
		self.hud.alignx = "center";
		self.hud.aligny = "top"; //top
		self.hud.horzalign = "center";
		self.hud.vertalign = "user_top"; //top
		self.hudtext.alignx = "center";
		self.hudtext.aligny = "top"; //top
		self.hudtext.horzalign = "center";
		self.hudtext.vertalign = "user_top"; //top
		self.hud.color = ( 1, 0.6, 0 );
		self.hudtext.color = ( 1, 1, 1 );

        self.hudtext settext("Test ^3subject^7 is advancing to phase");

		self.hud settext( level.round_number );
		self.hud.fontscale = 3.9;
		self.hud.x = 0;
		self.hud.y = 90;//90;
		self.hud.alpha = 0;
		self.hudtext.fontscale = 2.1;
		self.hudtext.x = 0;
		self.hudtext.y = 50;//50;
		self.hudtext.alpha = 0;
		flag_wait("initial_blackscreen_passed");
        wait 13.5;
        wait 1.5;
		self.hud fadeovertime( 1.5 );
		self.hud.alpha = 0.8;//0.8;
		self.hudtext fadeovertime( 1.5 );
		self.hudtext.alpha = 0.8;//0.8;
        
		wait 3;
		self.hudtext fadeovertime( 1 );
		self.hudtext.alpha = 0;
		self.hud moveovertime( 2 );
		self.hud.alignx = "right"; 
		self.hud.aligny = "bottom";
		self.hud.horzalign = "user_right";
		self.hud.vertalign = "user_bottom";
		self.hud.x = -35;
		self.hud.y = -15;
	}
}
...
flashroundnumber()
{
	...
	self.hud fadeovertime( 1);
	self.hud.alpha = 0;
	wait 1; //og 1
	self.hud.alignx = "center";
	self.hud.aligny = "top"; //top
	self.hud.horzalign = "center";
	self.hud.vertalign = "user_top"; //user_top
	self.hud settext( level.round_number );
	self.hud.alignx = "center";
	self.hud.aligny = "top"; //top
	self.hud.horzalign = "user_center";
	self.hud.vertalign = "user_top"; //user_top
	self.hud.x = 0;
	self.hud.y = 90;
	...
	self.hud.alpha = 1;
	self.hudtext fadeovertime( 1.5 );
	self.hudtext.alpha = 1; //1;
	wait 3;
	self.hudtext fadeovertime( 1 );
	self.hudtext.alpha = 0;
	wait 1;
	self.hud moveovertime( 2.2 );
	self.hud.x = -35;
	self.hud.y = -15;
	self.hud.alignx = "right";
	self.hud.aligny = "bottom";
            ...
            if( !player.machine_is_in_use )
			{
                if( distance( self.origin, player.origin ) <= 70  )
                {
					...
					if( type == "pap" && player usebuttonpressed() && !is_weapon_upgraded( currgun ) && can_upgrade_weapon( currgun ) && player.score >= cost && !player maps/mp/zombies/_zm_laststand::player_is_in_laststand() )
                    {
						...
						playfx(loadfx( "maps/zombie/fx_zombie_packapunch"), ( 59872.7, 141818, 88737.5 ), anglestoforward( ( 0, 290, 85  ) ) ); 
						wait 3;
                    	player.machine_is_in_use = 0;
					}
					else
...
lampFires()
{
    ...
    while( true )
    {
        ...
        level.transitfireloc[1] = (459.874, -2731.85, 158.014);
        level.transitfireloc[2] = (460.399, -2732.1, 196.601);
        level.transitfireloc[3] = (-16399.5, -998261, 56631.1);



        level.lampfirefx[0] = "fx_fire_fireplace_md";
        
        //level.lampfireloc[] = 
        level.lampfireloc[0] = (207.411, -774.074, 81.5862);
        level.lampfireloc[1] = (207.737, -773.599, 81.1377);
        level.lampfireloc[2] = (212.361, -769.693, 77.0285);
        level.lampfireloc[3] = (-189.092, -674.301, 84.2851);
        level.lampfireloc[4] = (-611963, -670804, 418956);
        level.lampfireloc[5] = (-198.635, -670.309, 81.0439);
        level.lampfireloc[6] = (-198.326, -669.828, 80.8396);
        level.lampfireloc[7] = (378395, -737736, 559073);
        level.lampfireloc[8] = (-198.377, -669.145, 81.7083);
        level.lampfireloc[9] = (-163.376, -1134.89, 79.7245);
        level.lampfireloc[10] = (-163.138, -1134.56, 79.5364);
        level.lampfireloc[11] = (201.834, -778.452, 79.7518);
        level.lampfireloc[12] = (199.28, -781.217, 80.0198);
        level.lampfireloc[13] = (231.038, -720.651, -60.544);
        level.lampfireloc[14] = (247.95, -710.737, -60.5377);
        level.lampfireloc[15] = (260.162, -717.709, -60.417);
        level.lampfireloc[16] = (224.977, -739.65, -60.6533);
        level.lampfireloc[17] = (241.221, -1334.18, -27.9585);
        level.lampfireloc[18] = (240.425, -1334.18, -40.6062);
        level.lampfireloc[19] = (240.561, -1334.18, -55.9061);
        level.lampfireloc[20] = (243.698, -1306.15, -60);
        level.lampfireloc[21] = (227.398, -1290.97, -60);
        level.lampfireloc[22] = (258.273, -1309.11, -60);
        level.lampfireloc[23] = (215.905, -1391.54, 85.2226);

        ////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////

        level.smallsparkfxa[0] = "fx_mp_elec_spark_burst_xsm_thin_runner";

        level.smallsparkloca[0] = (-197.931, -668.792, 80.8973);
        level.smallsparkloca[1] = (488502, -553072, 674891);
        level.smallsparkloca[2] = (493809, -534835, 685642);
        level.smallsparkloca[3] = (228987, -609545, 758960);
        level.smallsparkloca[4] = (239.076, -725.42, -39.0366);
        level.smallsparkloca[5] = (248.837, -723.671, -41.6659);
        level.smallsparkloca[6] = (253.058, -717.33, -60.4804);
        level.smallsparkloca[7] = (-198.561, -671.639, 79.8968);
        level.smallsparkloca[8] = (542679, -450243, 709070);
        level.smallsparkloca[9] = (-176.465, -614.364, -60);
        level.smallsparkloca[10] = (-177.682, -620.822, -60);
        level.smallsparkloca[11] = (-176.246, -619.479, -60);
        level.smallsparkloca[12] = (-170.907, -611.83, -60);
        
        ///////////////////////////////////////////////////////////////////////

        level.sparkwirefx[0] = "fx_elec_spark_wire_xsm_runner";

        
        level.sparkwireloc[0] = (482918, -481879, 731152);
        level.sparkwireloc[1] = (638209, -28150.8, 769348);
        level.sparkwireloc[2] = (240.304, -723.377, -44.4737);
        level.sparkwireloc[3] = (248.114, -724.047, -52.8102);
        level.sparkwireloc[4] = (-171.331, -630.704, -60);
        level.sparkwireloc[5] = (-177.038, -608.851, -60);
        level.sparkwireloc[6] = (-178.264, -615.18, -60);
        ...
        level.transformerloc[3] = (-228.881, -1836.52, 247.878);
        level.transformerloc[4] = (-234.955, -1821.22, 212.643);
    ...
    //foreach( fireloc in level.lampfireloc ) //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021
    //{
    //   playfx( level._effect[fireplace], (fireloc) );
    //}

    wait .2;

    /* //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021
    foreach( smallsparkloc in level.smallsparkloca )
    {
        playfx( level._effect[smallspark], (smallsparkloc ) );
    }

    wait .05;
    foreach( wireloc in level.sparkwireloc ) //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021 //commented out 13/12/2021
    {
...
randomfiresanddust()
{
    ...
    while (true)
    {
        ...
        //level.sand[9] = (-410.262, -1649.69, -60);
        

        level.fires[0] = "fx_fire_fireplace_md"; //might be bnker??
...
towerFX() //clocktower, not rockettower
{
    ...
    while ( true )
    {
        ...
        level.transitloc[15] = (-162.232, -1131.1, 74.2552);
        level.transitloc[16] = (2776.66, 16212.1, -2948.8);
        level.transitloc[17] = (1643.44, 941.186, -40.2681);
        level.transitloc[18] = (2391.95, 479.51, -140.75);
        level.transitloc[19] = (798.339, -465.845, -23.875);
        level.transitloc[20] = (787.068, -409.72, 12.8477);
        level.transitloc[21] = (-863.005, -480.975, -26.802);
        level.transitloc[22] = (-870.558, -338.932, -64);
        level.transitloc[23] = (-898.165, -267.527, -59.2952);
        //level.transitloc[] = 
        ...
        level.towerloc[6] = (-1710.93, 9663.62, -3681.84);
        level.towerloc[7] = (-458.736, 1466.82, 168.09);
        ...
        level.towerloc1[2] = (-308.056, 1510, 457.349);
        ...
        level.smokesloc[4] = (-617.336, 1488.99, -226.763);
        //level.smokesloc[] = 
